...
- **批量导入支持模型选择 + 分组处理**（v1.9.3 / v1.9.4）：导入弹窗可选「⚡ 快速模式」（Gemini Flash Lite + 并发 5）或「🎯 高质量模式」（GLM 5.2 + 并发 5），选择持久化到 localStorage。v1.9.4 起后端改为分组串行：30 词分 6 组（每组 5 个），组内并发，组间等 5 秒，避免触发 API 限流
- **词库批量删除**（v1.9.5）：词库页「批量管理」选择模式，支持勾选单个 / 多个 / 全选当前页，二次确认后一次性删除（上限 200）；后端级联清理复习历史（histories）与周计划任务 `linked_word_ids` 关联引用
...
- **全面安全与防刷加固**（v1.9.9）：所有 Supabase 数据表开启 RLS 行级隔离；密码加密升级为加盐 `scrypt` 并支持旧版本平滑自动迁移；新增认证与 AI 接口限流中间件；生产环境屏蔽调试接口；全量换用 `crypto.randomUUID()` 与报错堆栈脱敏
- **短语支持**（v1.10）：词条「含空格即短语」，不加 type 字段。批量导入分隔符改为换行/逗号/分号（空格不再是分隔符，短语保持完整，解析逻辑抽到 `src/lib/wordText.ts`）；后端词条清洗内部空格归一（多空格查重视为同一条）；拼写测试宽松比对（忽略大小写/多余空格/首尾标点）+ 掩码保留词边界；辨义选择模式自动跳过短语（本地词典查不到，题型会退化）——周计划自定义会话同理，被跳过的短语不参与 SRS 结算（两层词表快照设计防泄漏）；同版本顺带修复 v1.9.6 惰性加载 bug（`generate-distractors` 预检查先于词典加载，辨义模式首次请求必 500）
...
│   │   ├── WordList.tsx   # 词库管理（增删改查 + 批量导入 + 导入模式选择 v1.9.3 + 导入进度条 + v1.8.2：语言筛选下空词库温和提示 + v1.9.5：批量删除选择模式）
...
│       ├── wordText.ts        # 词条文本纯函数（v1.10：短语识别/拼写宽松比对归一/词边界掩码/批量导入解析）
