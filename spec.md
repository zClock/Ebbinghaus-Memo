...
- 批量导入**分组串行处理**（v1.9.4）：30 词分为 6 组（每组 5 个），组内并发执行，组间等待 5 秒，分散 API 压力避免限流
- 批量导入**字典 API 与 AI 并行执行**（v1.9.3）：单词耗时减半
...
- 🔴 性能策略调整：`/api/words/import-batch` 从"一次性并发全部"改为"分组串行"
  - 30 词分为 6 组（每组 5 个），组内并发 5，**组间等待 5 秒**
  - 目的：分散 AI API 压力，避免触发 QPS 限流（Gemini 免费版 10 RPM）
  - 常量：`GROUP_SIZE = 5`、`GROUP_DELAY_MS = 5000`
- 🟢 UX：前端进度提示文案更新为「分组处理 · 每组 5 词组间等 5 秒」
